...
function seededRandom(seed) {
  // Ensure seed is a string and not empty
...
export default function DummyReviewsSection({ product } = {}) {
  const reviews = useMemo(() => {
    const productId = product?._id || "default";
    const rng = seededRandom(productId);
    const count = Math.min(product?.reviewCount || 3, 6);
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      const rating = Math.max(
        3,
        Math.min(5, Math.round((product?.rating || 4) + (rng() - 0.5) * 2)),
      );
      const d = new Date();
